...
from LLMCache.utils import import_pymilvus
...
class Milvus(VectorBase):
    ...
    def _create_local(self, port, local_data):
        from LLMCache.utils import import_milvus_lite  # pylint: disable=import-outside-toplevel
        import_milvus_lite()
    ...
    def _create_collection(self, collection_name):
        if not utility.has_collection(collection_name, using=self.alias):
            ...
            llmcache_log.warning("The %s collection already exists, and it will be used directly.", collection_name)
            self.col = Collection(
        ...
        if len(self.col.indexes) == 0:
            try:
                llmcache_log.info("Attempting creation of Milvus index.")
                self.col.create_index("embedding", index_params=self.index_params)
                llmcache_log.info("Creation of Milvus index successful.")
            except MilvusException as e:
                llmcache_log.warning("Error with building index: %s, and attempting creation of default index.", e)
