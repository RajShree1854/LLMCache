...
from LLMCache.manager.scalar_data.base import CacheStorage, CacheData, Question, QuestionDep, Answer
from LLMCache.utils import import_boto3
...
class DynamoStorage(CacheStorage):
    ...
    def create(self):
        ...
        if not self._does_table_already_exist_and_is_active("LLMCache_questions"):
            self._dynamo.create_table(
                TableName = "LLMCache_questions",
                KeySchema = [
        ...
        if not self._does_table_already_exist_and_is_active("LLMCache_reports"):
            self._dynamo.create_table(
                TableName = "LLMCache_reports",
                KeySchema = [
    ...
    def batch_insert(self, all_data: List[CacheData]) -> List[str]:
        ...
        """
        table = self._dynamo.Table("LLMCache_questions")
    ...
    def report_cache(
        ...
        table = self._dynamo.Table("LLMCache_reports")
        table.wait_until_exists()
