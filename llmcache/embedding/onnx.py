...
class Onnx(BaseEmbedding):
    ...
    Example:
        .. code-block:: python
            ...
            encoder = Onnx(model='LLMCache/paraphrase-albert-onnx')
            embed = encoder.to_embeddings(test_sentence)
    """

    def __init__(self, model="LLMCache/paraphrase-albert-onnx"):
        tokenizer_name = "LLMCache/paraphrase-albert-small-v2"
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
        self.model = model
        onnx_model_path = hf_hub_download(repo_id=model, filename="model.onnx")
        self.ort_session = onnxruntime.InferenceSession(onnx_model_path)
        config = AutoConfig.from_pretrained(
            "LLMCache/paraphrase-albert-small-v2"
        )
