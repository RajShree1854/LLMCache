...
from LLMCache.processor.post import temperature_softmax, LlmVerifier
from LLMCache.utils.error import NotInitError
from LLMCache.utils.log import llmcache_log
from LLMCache.utils.time import time_cal
...
def adapt(llm_handler, cache_data_convert, update_cache_callback, *args, **kwargs):
    ...
    if cache_enable:
        try:
            ...
            llmcache_log.warning("failed to save the data to cache, error: %s", e)
    return llm_data
...
async def aadapt(
    ...
    if cache_enable:
        try:
            ...
            llmcache_log.error("failed to save the data to cache", exc_info=True)
    return llm_data
...
def _summarize_input(text, text_length):
    ...
    from LLMCache.processor.context.summarization_context import (
...
def cache_health_check(vectordb, cache_dict):
    ...
    if not flag:
        llmcache_log.critical("Cache Store and Vector Store are out of sync!!!")
