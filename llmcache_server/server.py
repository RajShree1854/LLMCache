...
from LLMCache import cache, Cache
from LLMCache.adapter import openai
from LLMCache.adapter.api import (
    get,
    put,
    init_similar_cache,
...
from LLMCache.utils import import_fastapi, import_pydantic, import_starlette
...
async def chat(request: Request):
    if openai_cache is None:
        raise HTTPException(
            status_code=500,
            detail=f"the LLMCache server doesn't open the openai completes proxy",
        )
...
def main():
    ...
    parser.add_argument(
        "-d", "--cache-dir", default="llmcache_data", help="the cache data dir"
    )
